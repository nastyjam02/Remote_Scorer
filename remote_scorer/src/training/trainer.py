...
class Trainer:
    ...
    def __init__(self, model, train_loader, val_loader, loss_fn, optimizer, scheduler, model_config, data_config, training_config, swanlab_config, processor=None):
        ...
        # Loss 权重配置
        self.random_negative_weight = 0.5  # 基础区分度
        self.explicit_negative_weight = 2.0  # 强调区分细节错误
        self.other_negative_weight = 0.5    # 利用其余负样本
    ...
    def validate_epoch(self, epoch: int) -> dict:
        ...
        correct_explicit_hard_predictions = 0 #显式难例准确率
        correct_other_predictions = 0         #其他负例准确率
        
        total_samples = 0
        total_other_pairs = 0                 #其他负例对总数
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_loader, desc=f"验证 Epoch {epoch}"):
                ...
                loss = loss_random + loss_explicit + loss_other # 验证集 Loss
                total_loss += loss.item()
                
                #计算Global Random Accuracy
                ...
                #计算 Explicit Hard 准确率 (Explicit Hard Accuracy)
                ...
                #计算 Other Negatives 准确率 (Local Random Accuracy)
    ...
    def save_checkpoint(self, dirname: str, epoch: int, metrics: dict):
        ...
        #保存模型权重 (委托给 Model 类的 save_pretrained)
        self.model.save_pretrained(save_dir)
        
        #保存训练器状态 (依然使用 torch.save)
        trainer_state = {
            'epoch': epoch,
