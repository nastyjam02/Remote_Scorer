...
class CaptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 处理路径：如果包含完整路径，尝试提取文件名并拼接到 image_root
        # 或者直接使用 image_root + basename
...
def main():
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            # logits_per_image: [batch_size, batch_size] (CLIP computes all pairs)
            # We only want diagonal (image_i matches text_i)
            logits_per_image = outputs.logits_per_image
            
            # SigLIP also returns logits_per_image
            
            # Extract diagonal elements for matched pairs
            # diag_logits: [batch_size]
...
python batch_score.py \
  --json_input "/data/oceanus_ctr/j-chenrui5-jk/Score/三元组数据-2000/qwen3_caption/image_descriptions_t_07_2000_3.json" \
  --json_output "/data/oceanus_ctr/j-chenrui5-jk/Score/三元组数据-2000/clipscore/image_descriptions_t_07_2000_3_scored.json"\
