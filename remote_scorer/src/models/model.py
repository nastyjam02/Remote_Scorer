"""
基于 Qwen2.5-VL 的双塔（Dual-Tower）图文编码器模型
"""
import os
import torch
...
class ProjectionHead(nn.Module):
    """
    将backbone网络输出的特征投影到共享的嵌入空间并支持动态层数构建。
    """
    ...
    def _init_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)  # 使用了 Xavier Uniform 初始化权重
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, last_hidden_state: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        ...
        # 处理 Padding，将 Padding 位置的分数设为极小值 (-inf)
...
class ImageTextEncoder(nn.Module):
    """
    把原本用于生成的模型拆开了，分别作为独立的特征提取器使用，分别编码图像和文本到同一语义空间。
    """
    ...
    def from_pretrained(cls, load_directory: str, config=None):
        ...
        #  初始化模型结构
        # backbone 从原始的预训练模型路径加载（基础底座）
        # 然后再加载 LoRA 权重
        
        model = cls(config)
        
        #  如果使用了 LoRA，显式加载 LoRA 权重
        if getattr(config, 'use_lora', False):
