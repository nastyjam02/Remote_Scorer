...
class ImageTextDataset(Dataset):
    ...
    def __init__(self, image_paths: list, texts: list, image_size: int = 512):
        ...
        Args:
            image_paths: 图片路径列表
            texts: 对应的文本列表
            image_size: 图片 Resize 的大小。必须强制统一尺寸，否则无法进行 Batch 推理。
        """
...
def collate_fn(batch):
    """
    自定义 collate_fn，因为 batch 中包含 PIL.Image 对象，默认的 collate 无法处理
    """
...
class SimilarityScorer:
    ...
    def score(self, image_path: str, text: str) -> float:
        ...
        try:
            image = Image.open(image_path).convert("RGB")
            # 同样需要 resize 以匹配模型预期的输入逻辑 (虽然单张不做 resize 也能跑，但为了保持一致性)
            # 这里假设使用默认的 512
    ...
    def score_batch(self, images: list, texts: list) -> list:
        ...
        try:
            ...
            # 3. 推理
            with torch.no_grad():
                # 注意：encode_image 内部依赖 view(batch_size, -1, hidden), 
                # 这要求所有图片产生的 token 数必须一致，这由 Dataset 中的 resize 保证
                ...
                # 4. 计算余弦相似度
                # image_embedding: [B, D], text_embedding: [B, D]
